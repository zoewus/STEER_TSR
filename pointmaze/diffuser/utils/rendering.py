...
class PointMazeRenderer(Maze2dRenderer):
    def __init__(self, env):
        self.env_name = env
        self.env = load_environment(env)
        self.observation_dim = np.prod(self.env.observation_space.shape)
    ...
    def render_background(self, ax):
        maze_map = self.env.maze_map
        from matplotlib.patches import Rectangle
        maze_height, maze_width = maze_map.shape
        for i in range(maze_height):
            for j in range(maze_width):
                if maze_map[i, j] == 1:  # Assuming 1 represents walls
                    center_x, center_y = self.env.ij_to_xy((i, j))
                    rect = Rectangle((center_x - 0.5 * self.env._maze_unit, center_y - 0.5 * self.env._maze_unit), self.env._maze_unit, self.env._maze_unit, color="gray")
                    ax.add_patch(rect)
    ...
    def renders(self, observations, conditions=None, title=None):
        ...
        # ax.set_xlim(-10, 60)
        # ax.set_ylim(-10, 60)
        ax = self.render_background(ax)
        path_length = len(observations)
        colors = plt.cm.jet(np.linspace(0,1,path_length))
        ax.plot(observations[:,0], observations[:,1], c='black', zorder=10)
        # s = data_radius_to_scatter_size(0.7,ax)
        ax.scatter(observations[:,0], observations[:,1], c=colors, zorder=20)
        plt.axis('off')
        plt.title(title)
        img = plot2img(fig, remove_margins=self._remove_margins)
        return img
        
def data_radius_to_scatter_size(radius, ax):
    """
    Convert a circle radius in data coordinates to the marker size (s) for plt.scatter.
    Returns s in points^2.
    """
    fig = ax.get_figure()
    trans = ax.transData.transform
    # radius in data coordinates -> two points in data -> pixels
    p1 = trans((0, 0))
    p2 = trans((radius, radius))
    pixel_radius = ((p2[0] - p1[0])**2 + (p2[1] - p1[1])**2)**0.5 / 2
    import math
    return (math.pi * pixel_radius ** 2)
